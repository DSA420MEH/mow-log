 ...
 *
 * All functions read directly from useStore.getState() — they are PURE
 * FUNCTIONS, not React hooks. Components should call them inside useMemo
...
export function computeClientSummary(clientId: string): ClientSummary {
    const state = useStore.getState();
...
export function computeDailyProfit(dateStr: string): DailyProfit {
    const state = useStore.getState();
    const { clients, sessions, gasLogs, maintenanceLogs } = state;
...
export function computeEquipmentAlerts(): EquipmentAlert[] {
    const state = useStore.getState();
